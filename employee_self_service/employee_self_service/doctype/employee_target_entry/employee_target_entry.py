...
class EmployeeTargetEntry(Document):
    ...
    def perform_calculations(self):
        ...
        elif self.selector == "Customer Group":
            self._calculate_customer_group_progress()
        else:
    ...
    def _calculate_customer_group_progress(self):
        """Calculate progress for customer group based targets."""
        customer_group_targets = self.get("customer_group_wise_target")
        if not customer_group_targets:
            self.total_target = self.total_achieved = self.progress = 0
            return
        ...
        for row in customer_group_targets:
            target = row.target or 0
            achieved = row.achieved or 0
            considered = min(achieved, target)
    ...
    def get_template_groups(self):
        ...
        if not self.target_template:
            frappe.throw(_("No target template selected"))

        try:
            template_doc = self._get_cached_template()
            # Decide which child table to use
            if self.selector == "Item Group":
                child_table = "item_group_list"
            elif self.selector == "Customer Group":
                child_table = "customer_group_list"
            else:
                return []

            rows = template_doc.get(child_table, [])

            return {
                "child_table": f"{frappe.scrub(self.selector)}_wise_target",
                "rows": rows,
            }
        except Exception as e:
            frappe.log_error(
                title="Error fetching template template groups",
