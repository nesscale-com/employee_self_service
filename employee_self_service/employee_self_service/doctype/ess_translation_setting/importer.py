...
class Importer:
	def __init__(
		self, doctype, ess_translation_setting=None, file_path=None, import_type=None, console=False, use_sniffer=False
	):
		self.doctype = doctype
		self.console = console
		self.use_sniffer = use_sniffer

		self.ess_translation_setting = ess_translation_setting
		if not self.ess_translation_setting:
			self.ess_translation_setting = frappe.get_doc(doctype="ESS Translation Setting")
			if import_type:
				self.ess_translation_setting.import_type = import_type

		self.template_options = frappe.parse_json(self.ess_translation_setting.template_options or "{}")
		self.import_type = self.ess_translation_setting.import_type

		self.import_file = ImportFile(
			doctype,
			file_path or ess_translation_setting.google_sheets_url or ess_translation_setting.import_file,
	...
	def get_data_for_import_preview(self):
		...
		out.import_log = frappe.get_all(
			...
			filters={"ess_translation_request": self.ess_translation_setting.name},
			order_by="log_index",
			limit=10,
		)
	...
	def before_import(self):
		...
		self.ess_translation_setting.db_set("template_warnings", "")
	...
	def import_data(self):
		...
		if warnings:
			if self.console:
				...
				self.ess_translation_setting.db_set("template_warnings", json.dumps(warnings))
			return
		...
		if (
			self.ess_translation_setting.status in ("Partial Success", "Error")
			and len(import_log) >= self.ess_translation_setting.payload_count
		):
			# remove previous failures from import log only in case of retry after partial success
			import_log = [log for log in import_log if log.get("success")]
			frappe.db.delete("ESS Translation Request Log", {"success": 0, "ess_translation_request": self.ess_translation_setting.name})

		# get successfully imported rows
		imported_rows = []
		for log in import_log:
			log = frappe._dict(log)
			if log.success or len(import_log) < self.ess_translation_setting.payload_count:
		...
		batch_size = frappe.conf.ess_translation_setting_batch_size or 1000
		...
		for batch_index, batched_payloads in enumerate(frappe.utils.create_batch(payloads, batch_size)):
			for i, payload in enumerate(batched_payloads):
				...
				if set(row_indexes).intersection(set(imported_rows)):
					...
					if total_payload_count > 5:
						frappe.publish_realtime(
							"ess_translation_setting_progress",
							{
								"current": current_index,
								"total": total_payload_count,
								"skipping": True,
								"ess_translation_setting": self.ess_translation_setting.name,
							},
				...
				try:
					...
					create_import_log(
						self.ess_translation_setting.name,
						log_index,
					...
					if self.ess_translation_setting.status != "Partial Success":
						self.ess_translation_setting.db_set("status", "Partial Success")
		...
		if self.console:
			...
			self.ess_translation_setting.db_set("status", status)
	...
	def insert_record(self, doc):
		...
		new_doc.flags.updater_reference = {
			"doctype": self.ess_translation_setting.doctype,
			"docname": self.ess_translation_setting.name,
			"label": _("via ESS Translation Setting Import"),
		}

		new_doc.insert()
		if meta.is_submittable and self.ess_translation_setting.submit_after_import:
			new_doc.submit()
		return new_doc
...
def get_df_for_column_header(doctype, header):
	...
	df_by_labels_and_fieldname = frappe.cache.hget(
		"ess_translation_setting_column_header_map", doctype, generator=build_fields_dict_for_doctype
	)
...
def create_import_log(ess_translation_setting, log_index, log_details):
	frappe.get_doc(
		{
			"doctype": "ESS Translation Request Log",
			"log_index": log_index,
			"success": log_details.get("success"),
			"ess_translation_request": ess_translation_setting,
