...
frappe.ui.form.on("ESS Translation Setting", {
    setup(frm) {
		frappe.realtime.on("ess_translation_setting_refresh", ({ ess_translation_setting }) => {
			frm.import_in_progress = false;
			if (ess_translation_setting !== frm.doc.name) return;
			frappe.model.clear_doc("ESS Translation Setting", frm.doc.name);
			frappe.model.with_doc("ESS Translation Setting", frm.doc.name).then(() => {
				frm.refresh();
			});
		});
		frappe.realtime.on("ess_translation_setting_progress", (data) => {
			frm.import_in_progress = true;
			if (data.ess_translation_setting !== frm.doc.name) {
				return;
			}
	...
	refresh(frm) {
		...
		if (frm.doc.status.includes("Success")) {
			frm.add_custom_button(__("Go to {0} List", [__("ESS Translation Request")]), () =>
				frappe.set_route("List", "ESS Translation Request")
			);
		}
	},
	...
	show_import_status(frm) {
		frappe.call({
			method: "employee_self_service.employee_self_service.doctype.ess_translation_setting.ess_translation_setting.get_import_status",
			args: {
				ess_translation_setting: frm.doc.name,
			},
			callback: function (r) {
	...
	start_import(frm) {
		frm.call({
			...
			args: { ess_translation_setting: frm.doc.name },
	...
	download_template(frm) {
		frappe.require("translation_import_tools.bundle.js", () => {
			frm.data_exporter = new frappe.translation_import.DataExporter(
				"ESS Translation",
				frm.doc.import_type
			);
		});
	...
	show_import_preview(frm, preview_data) {
		...
		if (frm.import_preview && frm.import_preview.doctype === "ESS Translation Request") {
		...
		frappe.require("data_import_tools.bundle.js", () => {
			frm.import_preview = new frappe.data_import.ImportPreview({
				...
				doctype: "ESS Translation Request",
				preview_data,
				import_log,
				frm,
				events: {
	...
	export_errored_rows(frm) {
		open_url_post(
			"/api/method/employee_self_service.employee_self_service.doctype.ess_translation_setting.ess_translation_setting.download_errored_template",
			{
				ess_translation_setting: frm.doc.name,
			}
		);
	...
	export_import_log(frm) {
		open_url_post(
			"/api/method/employee_self_service.employee_self_service.doctype.ess_translation_setting.ess_translation_setting.download_import_log",
			{
				ess_translation_setting: frm.doc.name,
			}
		);
				...
				let message = warnings_by_row[row_number]
					.map((w) => {
						if (w.field) {
							let label =
								w.field.label +
								(w.field.parent !== "ESS Translation Request"
									? ` (${w.field.parent})`
									: "");
	...
	render_import_log(frm) {
		frappe.call({
			method: "employee_self_service.employee_self_service.doctype.ess_translation_setting.ess_translation_setting.get_import_logs",
			args: {
				ess_translation_setting: frm.doc.name,
			},
			callback: function (r) {
	...
	show_import_log(frm) {
		...
		frappe.call({
			...
			args: {
				...
				filters: {
					ess_translation_request: frm.doc.name,
				},
			},
			callback: function (r) {
				let count = r.message;
