    ...
    def get_all_exportable_fields(self):
        ...
        for key, fieldnames in self.export_fields.items():
            if key == self.doctype:
                ...
                exportable_fields[key] = self.get_exportable_fields(
                    child_doctype, fieldnames
                )
    ...
    def add_data_row(self, doctype, parentfield, doc, rows, row_idx):
        ...
        for i, df in enumerate(self.fields):
            if df.parent == doctype:
                if (
                    df.is_child_table_field
                    and df.child_table_df.fieldname != parentfield
                ):
                    continue
                ...
                *list(
                    {
                        ...
                        if df.parent == child_table_doctype
                    }
                ),
            ]
            data = frappe.get_all(
