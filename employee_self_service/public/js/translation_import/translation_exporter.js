frappe.provide("employee_self_service.translation_import");

frappe.translation_import.DataExporter = class DataExporter {
	...
	export_records() {
		...
		if (values.export_records === "by_filter") {
			filters = this.get_filters(); // from UI filters
		} else {
			filters = {};
		}

		// 🔁 Custom filter logic for ESS Translation
		if (this.doctype === "ESS Translation") {
			// Get destination_language from filter dialog if available
			let destination_language_filter = (filters || []).find(
				(f) => f[0] === "destination_language"
			);

			// If not in filters, you can default it or skip
			if (!destination_language_filter) {
				frappe.msgprint("Please apply a Destination Language filter.");
				return;
			}

			let destination_language = destination_language_filter[2];

			// Now call server to get ESS Word names first, then export
			frappe.call({
				...
				callback: (r) => {
					if (!r.message || r.message.length === 0) {
						frappe.msgprint("No ESS Words found.");
						return;
					}

					let ess_words = r.message.map((w) => w.name);

					// Build export_filters
					let export_filters = {
					...
					open_url_post(method, {
						...
						export_filters: JSON.stringify(export_filters),
					});
				},
			});
		} else {
			// Default export for other Doctypes
			open_url_post(method, {
				doctype: this.doctype,
				file_type: values.file_type,
				export_records: values.export_records,
