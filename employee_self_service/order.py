...
def create_random_sales_orders(number_of_orders):
    try:
        ...
        customers = ["Digital World Ltd.", "Future Tech Enterprises", "Advanced Computing Corp.","Innovate IT Services"]

        # Fetch all item codes
        item_codes = [item.name for item in frappe.get_all('Item', filters={"is_sales_item": 1, "disabled": 0}, fields=['name'])]
