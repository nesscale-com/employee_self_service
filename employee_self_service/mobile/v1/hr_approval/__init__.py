    ...
    try:
        ...
        if not workflow:
            ...
            leave_applications = frappe.get_list(
                ...
                filters=filters,
                fields=[
                    "employee_name", "name", "posting_date", "from_date", "to_date",
                    "leave_type", "employee", "total_leave_days", "description",
                    "status", "'0' as 'workflow_active'"
                ],
                order_by="posting_date desc",
                start=start,
                page_length=page_length
            )
            return gen_response(200, "Leave Application Get Successfully", leave_applications)
        ...
        leave_applications = frappe.get_list(
            ...
            fields=[
                "employee_name", "name", "posting_date", "from_date", "to_date",
                "leave_type", "employee", "total_leave_days", "description",
                "workflow_state as 'status'", "'1' as 'workflow_active'"
            ],
            order_by="posting_date desc",
        ...
        for doc in leave_applications:
            if doc.get("status"):
                transitions = get_transitions(frappe.get_doc("Leave Application", doc["name"]))
                if transitions:
                    actual_leave_applications.append(doc)
                    if len(actual_leave_applications) == page_length:
                        break

        return gen_response(200, "Team Leave Application Get Successfully", actual_leave_applications)
    ...
    try:
        ...
        for doc in claims:
            if doc.get("approval_status"):
                transitions = get_transitions(frappe.get_doc("Expense Claim", doc["name"]))
                if transitions:
                    updated_expense_claim_list.append(doc)
                    if len(updated_expense_claim_list) == page_length:
                        break

        return gen_response(200, "Team Expense Claim Get Successfully", updated_expense_claim_list)
    except Exception as e:
        return exception_handler(e)
...
def get_team_expense_details(expense_id):
    try:
        ...
        expense_doc = json.loads(
            frappe.get_doc("Expense Claim", expense_id).as_json()
        )
        if is_workflow:
        ...
        for row in expense_doc.get("expenses"):
            row["amount"] = fmt_money(row.get("amount"),currency=global_defaults.get("default_currency"))
