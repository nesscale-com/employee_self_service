...
def get_conference_list():
    try:
        conference_list = frappe.get_all(
            "Training Event", filters={"type": "Conference"}, fields=["name"]
        )
        return gen_response(
            200, "Conference List getting Successfully", conference_list
        )
    except frappe.PermissionError:
        return gen_response(500, "Not permitted read user")
    except Exception as e:
...
def get_training_list():
    try:
        conference_list = frappe.get_all(
            "Training Event",
            filters=[["Training Event", "type", "!=", "Conference"]],
            fields=["name"],
        )
        return gen_response(
            200, "Conference List getting Successfully", conference_list
