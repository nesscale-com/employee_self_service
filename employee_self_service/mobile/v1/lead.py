...
def get_lead_details(name):
    try:
        ...
        lead_doc["creation_date"] = datetime.strptime(
            str(lead_doc.get("creation")), "%Y-%m-%d %H:%M:%S.%f"
        ).strftime("%d-%m-%Y")
...
def user_assignment(**data):
    try:
        validate_required_fields(
            data, ["reference_type", "reference_name", "assigned_to"]
        )
        for user in data.get("assigned_to"):
            todo = frappe.get_doc(
                {
...
def get_options(**data):
    try:
        ...
        for df in meta.fields:
            if (
                df.fieldname == data.get("fieldname")
                and df.fieldtype == "Select"
                and df.options
            ):
                options = [
                    {"name": opt.strip()}
...
def get_industry_type_list():
    try:
        ...
        return gen_response(
            200, "Industry Type list fetched successfully.", industry_type
        )
    except frappe.PermissionError:
...
def get_lead_open_activities(name):
    try:
        ...
        for event in open_activities.get("events", []):
            event["starts_on"] = (
                event.get("starts_on").strftime("%Y-%m-%d")
                if event.get("starts_on")
                else None
            )
            event["ends_on"] = (
                event.get("ends_on").strftime("%Y-%m-%d")
                if event.get("ends_on")
                else None
            )
        return gen_response(
            200, "Lead Open activity list fetched successfully.", open_activities
        )
    except frappe.PermissionError:
...
def create_lead_task(**data):
    try:
        ...
        todo.insert()
        return gen_response(
            200, "Lead Task created successfully.", {"task_name": todo.name}
        )
    except frappe.PermissionError:
        ...
        event = frappe.get_doc(
            {
                ...
                "event_participants": [
                    {"reference_doctype": "Lead", "reference_docname": data.get("name")}
                ],
            }
        )
        event.insert()
        return gen_response(
            200, "Lead event created successfully.", {"event_name": event.name}
        )
    except frappe.PermissionError:
...
def close_activity(**data):
    try:
        ...
        frappe.db.set_value(
            data.get("reference_type"), data.get("reference_name"), "status", "Closed"
        )
