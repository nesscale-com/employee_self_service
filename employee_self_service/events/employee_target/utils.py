    ...
    for target_group in target_groups:
        try:
            ...
            descendants = get_descendants_of(
                "Item Group", target_group, ignore_permissions=True
            )
...
def get_customer_group_hierarchy(target_groups):
    ...
    for target_group in target_groups:
        try:
            # Get all descendants of the target group
            descendants = get_descendants_of(
                "Customer Group", target_group, ignore_permissions=True
            )
            expanded_groups.update(descendants)
        except Exception:
            # Fallback: if nestedset fails, the target group itself is still included
...
def create_target_log(
    ...
    try:
        ...
        if group_field:
            duplicate_check_filters[group_field] = (
                group_value if group_value else ["is", "not set"]
            )
