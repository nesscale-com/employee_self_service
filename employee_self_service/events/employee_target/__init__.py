...
def create_sales_person_target_log(doc, method=None):
    ...
    for employee in employees:
        try:
            ...
            active_targets = get_active_target_entries(
                employee, doc.doctype, transaction_date
            )
            ...
            for target_doc in active_targets:
                ...
                if selector == "Item Group" or (
                    has_item_groups and not has_customer_groups
                ):
                    # Handle item group only targeting
                    handle_item_group_target(
                        doc, target_doc, employee, metric, transaction_date
                    )
                elif selector == "Customer Group" or (
                    has_customer_groups and not has_item_groups
                ):
